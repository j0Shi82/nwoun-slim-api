...
use \Exception;
use \PDO;
use App\Schema\Crawl\ArticleTags\ArticleTags as ChildArticleTags;
 ...
 *
 *
 * @method     ChildArticleTagsQuery orderByArticleId($order = Criteria::ASC) Order by the article_id column
 * @method     ChildArticleTagsQuery orderByTagId($order = Criteria::ASC) Order by the tag_id column
 *
 * @method     ChildArticleTagsQuery groupByArticleId() Group by the article_id column
 * @method     ChildArticleTagsQuery groupByTagId() Group by the tag_id column
 *
 * @method     ChildArticleTagsQuery leftJoin($relation) Adds a LEFT JOIN clause to the query
 * @method     ChildArticleTagsQuery rightJoin($relation) Adds a RIGHT JOIN clause to the query
 * @method     ChildArticleTagsQuery innerJoin($relation) Adds a INNER JOIN clause to the query
 *
 * @method     ChildArticleTagsQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
...
abstract class ArticleTagsQuery extends ModelCriteria
{
    ...
    public static function create(?string $modelAlias = null, ?Criteria $criteria = null): Criteria
    {
        ...
        $query = new ChildArticleTagsQuery();
        if (null !== $modelAlias) {
    ...
    public function findPk($key, ?ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ArticleTagsTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
     ...
     *
     * @return ChildArticleTags A model object, or null if the key is not found
     */
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            /** @var ChildArticleTags $obj */
            $obj = new ChildArticleTags();
            $obj->hydrate($row);
            ArticleTagsTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(ArticleTagsTableMap::COL_ARTICLE_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(ArticleTagsTableMap::COL_TAG_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            $cton1 = $this->getNewCriterion(ArticleTagsTableMap::COL_TAG_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function filterByArticleId($articleId = null, ?string $comparison = null)
    {
        if (is_array($articleId)) {
            ...
            if (isset($articleId['min'])) {
                $this->addUsingAlias(ArticleTagsTableMap::COL_ARTICLE_ID, $articleId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($articleId['max'])) {
                $this->addUsingAlias(ArticleTagsTableMap::COL_ARTICLE_ID, $articleId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByTagId($tagId = null, ?string $comparison = null)
    {
        if (is_array($tagId)) {
            $useMinMax = false;
            if (isset($tagId['min'])) {
                $this->addUsingAlias(ArticleTagsTableMap::COL_TAG_ID, $tagId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($tagId['max'])) {
                $this->addUsingAlias(ArticleTagsTableMap::COL_TAG_ID, $tagId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    {
        if ($articleTags) {
            $this->addCond('pruneCond0', $this->getAliasedColName(ArticleTagsTableMap::COL_ARTICLE_ID), $articleTags->getArticleId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(ArticleTagsTableMap::COL_TAG_ID), $articleTags->getTagId(), Criteria::NOT_EQUAL);
    ...
    public function doDeleteAll(?ConnectionInterface $con = null): int
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            ArticleTagsTableMap::clearRelatedInstancePool();
    ...
    public function delete(?ConnectionInterface $con = null): int
    {
        ...
        $criteria->setDbName(ArticleTagsTableMap::DATABASE_NAME);
        ...
        return $con->transaction(function () use ($con, $criteria) {
            ...
            ArticleTagsTableMap::removeInstanceFromPool($criteria);
